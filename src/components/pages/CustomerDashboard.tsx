...
import { LogOut, Package, Search, MapPin, Clock, Loader2, Plus, X, Check } from 'lucide-react';
...
const CustomerDashboard: React.FC = () => {
    ...
    const myOrders = useQuery(api.orders.listUserOrders, { userId: user?.id as any || "" });
    const availableServices = useQuery(api.services.listServices, selectedShop ? { shopId: selectedShop._id } : "skip");
            ...
            <main className="max-w-4xl mx-auto p-6 space-y-8">
                <section>
                    ...
                    <div className="space-y-4">
                        {myOrders === undefined ? (
                            ...
                            myOrders.map((order) => (
                                <div key={order._id} className="bg-white p-5 rounded-2xl border border-slate-200 flex items-center justify-between">
                                    <div>
                                        <h3 className="font-bold text-slate-800">{order.shopName}</h3>
                                    ...
                                    <div className="text-right">
                                        <span className={`px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider ${order.status === 'Ready' || order.status === 'collected'
                ...
                <section>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {shops === undefined ? (
                            <div className="col-span-full flex justify-center p-8"><Loader2 className="animate-spin text-[#69b8c4]" /></div>
                        ) : shops.length === 0 ? (
                            <div className="col-span-full text-center text-slate-400 py-10 italic">No shops found in your area yet.</div>
                        ) : (
                            shops.map((shop) => (
                                <div key={shop._id} className="bg-white p-6 rounded-2xl border border-slate-200 hover:border-[#69b8c4] transition-all group">
                                    <div className="flex justify-between items-start">
                                        <div>
                                            <h3 className="font-bold text-lg text-slate-800 group-hover:text-[#69b8c4] transition-colors">{shop.name}</h3>
                                            <p className="text-sm text-slate-500 flex items-center gap-1 mt-1">
                                                <MapPin size={14} /> {shop.latitude.toFixed(4)}, {shop.longitude.toFixed(4)}
                                            </p>
                                        </div>
                                        <button
                                            onClick={() => handleSelectShop(shop)}
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
                    <div className="bg-white w-full max-w-lg rounded-[2.5rem] overflow-hidden border border-slate-100 animate-in zoom-in-95 duration-300">
                        <div className="p-8 border-b border-slate-50 flex items-center justify-between bg-slate-50/50">
                            <div>
                                <h3 className="text-2xl font-black text-slate-800 italic uppercase tracking-tight">Select Service</h3>
                                <p className="text-[#69b8c4] font-bold text-sm italic">{selectedShop?.name}</p>
                            </div>
                            <button
                        ...
                        <div className="p-8 max-h-[60vh] overflow-y-auto space-y-4">
                            {availableServices === undefined ? (
                                <div className="flex justify-center p-20"><Loader2 className="animate-spin text-[#69b8c4]" size={32} /></div>
                                ...
                                availableServices.map((service) => (
                                    ...
                                    >
                                        <div>
                                            <h4 className="font-black text-slate-800 group-hover:text-[#69b8c4] transition-colors italic uppercase">{service.name}</h4>
                                            <p className="text-[#69b8c4] text-xl font-black italic">₱{service.price}</p>
                                        </div>
                        ...
                        <div className="p-8 bg-slate-50/50 text-center">
                            <p className="text-xs text-slate-400 font-bold italic uppercase tracking-widest">Select a service to finalize your order</p>
                        </div>
                    </div>
                </div>
            )}
